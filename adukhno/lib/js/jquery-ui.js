			...
			$.widget(
					"ui.draggable", $.ui.mouse, {
						...
						_mouseStop: function ( event ) {
							...
							if( (this.options.revert === "invalid" && !dropped) || (this.options.revert === "valid" && dropped) || this.options.revert === true || ($.isFunction( this.options.revert ) && this.options.revert.call(
											this.element,
											dropped )) ) {
								$( this.helper ).animate(
						...
						_getParentOffset: function () {
							...
							if( this.cssPosition === "absolute" && this.scrollParent[0] !== document && $.contains(
											this.scrollParent[0],
											this.offsetParent[0] ) ) {
						...
						_getRelativeOffset: function () {
							...
							return {
								top: p.top - ( parseInt( this.helper.css( "top" ), 10 ) || 0 ) + ( !scrollIsRootNode ?
										this.scrollParent.scrollTop() :
										0 ),
								left: p.left - ( parseInt(
										this.helper.css( "left" ),
										10 ) || 0 ) + ( !scrollIsRootNode ?
						...
						_setContainment: function () {
							...
							this.containment = [
								( parseInt(
										...
										10 ) || 0 ) + ( parseInt( c.css( "paddingLeft" ), 10 ) || 0 ),
					...
					switch( toleranceMode ) {
						...
						case "pointer":
							return isOverAxis(
											event.pageY,
											t,
											droppable.proportions().height ) && isOverAxis(
											event.pageX,
											l,
					...
					"ui.sortable", $.ui.mouse, {
						...
						_mouseStop: function ( event, noPropagation ) {
							...
							if( this.options.revert ) {
								...
								if( !axis || axis === "x" ) {
									animation.left = cur.left - this.offset.parent.left - this.margins.left + (this.offsetParent[0] === document.body ?
											0 :
											this.offsetParent[0].scrollLeft);
								}
								if( !axis || axis === "y" ) {
									animation.top = cur.top - this.offset.parent.top - this.margins.top + (this.offsetParent[0] === document.body ?
											0 :
						...
						serialize: function ( o ) {
							...
							$( items ).each(
									function () {
										...
										if( res ) {
											str.push(
													(o.key || res[1] + "[]") + "=" + (o.key && o.expression ?
															res[1] :
															res[2]) );
										}
									} );
							...
							if( !o.placeholder || o.placeholder.constructor === String ) {
								...
								o.placeholder = {
									...
									update: function ( container, p ) {
										...
										if( !p.height() ) {
											p.height(
													that.currentItem.innerHeight() - parseInt(
										...
										if( !p.width() ) {
											p.width(
													that.currentItem.innerWidth() - parseInt(
						...
						_setContainment: function () {
							...
							if( !(/^(document|window|parent)$/).test( o.containment ) ) {
								...
								this.containment = [
									co.left + (parseInt(
											$( ce ).css( "borderLeftWidth" ),
											10 ) || 0) + (parseInt(
											$( ce ).css( "paddingLeft" ),
											10 ) || 0) - this.margins.left,
									co.top + (parseInt(
											$( ce ).css( "borderTopWidth" ),
											10 ) || 0) + (parseInt(
											$( ce ).css( "paddingTop" ),
															...
															this.document.one(
																	"mousedown", function ( event ) {
																		if( event.target !== that.element[0] &&
																				event.target !== menuElement && !$.contains(
																						menuElement,
																						event.target ) ) {
																			that.close();
																		}
						...
						_resetButton: function () {
							...
							if( icons.primary || icons.secondary ) {
								if( this.options.text ) {
									buttonClasses.push(
											"ui-button-text-icon" + ( multipleIcons ?
													"s" :
			...
			var dialog = $.widget(
					"ui.dialog", {
						...
						_moveToTop: function ( event, silent ) {
							var moved = false,
									zIndicies = this.uiDialog.siblings( ".ui-front:visible" ).map(
											function () {
